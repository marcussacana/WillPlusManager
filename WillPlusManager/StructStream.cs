...
namespace AdvancedBinary {
    ...
    static class Tools {
        ...
        public static ulong GetStructLength<T>(T Struct, Encoding Encoding = null) {
                ...
                switch (field.FieldType.ToString()) {
                    ...
                    case Const.STRING:
                        if (!HasAttribute(field, Const.FSTRING)) {
                            try {
                                string Data = (string)field.GetValue(Struct);
                                if (string.IsNullOrEmpty(Data) || Encoding == null) {
                                    throw new Exception();
                                }
                                if (HasAttribute(field, Const.CSTRING) || HasAttribute(field, Const.UCSTRING))
                                    Data += '\x0';

                                Length += (uint)Encoding.GetByteCount(Data);
                            } catch {
